...
// All subdomains to test with their specific URLs
const subdomainsToTest = [
    {
        name: 'Main Site',
        baseUrl: 'https://www.asisaga.com',
        urls: [
            '/',
            '/about',
            '/contact',
    ...
    },
    {
        ...
        baseUrl: 'https://businessinfinity.asisaga.com',
        urls: [
            '/',
            '/features',
            '/trust',
    ...
    },
    {
        ...
        baseUrl: 'https://realmofagents.asisaga.com',
        urls: ['/']
    }
];
...
for (const subdomain of subdomainsToTest) {
    for (const vp of viewports) {
        for (const url of subdomain.urls) {
            test.describe(`${subdomain.name} - ${vp.name} viewport - ${url}`, () => {
                test(`should have no accessibility, overflow, or overlap issues`, async ({ page }) => {
                    await page.setViewportSize({ width: vp.width, height: vp.height });

                    try {
                        await page.goto(`${subdomain.baseUrl}${url}`, {
                            waitUntil: 'networkidle',
                            timeout: 30000
                        });
                    } catch (error) {
                        console.warn(`Could not load ${subdomain.baseUrl}${url}, skipping test`);
                        test.skip();
                        return;
                    }
                    ...
                    const axeResults = await new AxeBuilder({ page })
                        ...
                        .analyze();
                    expect(axeResults.violations, `Accessibility violations at ${subdomain.name} ${vp.name} ${url}`).toEqual([]);
                    ...
                    expect(layoutIssues, `Layout system issues at ${subdomain.name} ${vp.name} ${url}:\n${layoutIssues.join('\n')}`).toEqual([]);
                    ...
                    const overflowIssues = await page.evaluate(() => {
                        ...
                        return issues.slice(0, 5); // Limit output
                    });
                    expect(overflowIssues, `Overflow issues at ${subdomain.name} ${vp.name} ${url}:\n${overflowIssues.join('\n')}`).toEqual([]);
                    ...
                    const overlapIssues = await page.evaluate(() => {
                        ...
                        for (let i = 0; i < Math.min(elements.length - 1, 20); i++) { // Limit checks
                            const rect1 = elements[i].getBoundingClientRect();
                            if (rect1.width === 0 || rect1.height === 0) continue;

                            for (let j = i + 1; j < Math.min(elements.length, i + 10); j++) { // Limit comparisons
                        ...
                        return issues.slice(0, 5); // Limit to first 5 to avoid overwhelming output
                    });
                    expect(overlapIssues, `Overlap issues at ${subdomain.name} ${vp.name} ${url}:\n${overlapIssues.join('\n')}`).toEqual([]);
                    ...
                    const contrastIssues = await page.evaluate(() => {
                        ...
                        Array.from(textElements).slice(0, 50).forEach(el => { // Limit checks
                            const style = window.getComputedStyle(el);
                    ...
                    expect(contrastIssues, `Color contrast issues at ${subdomain.name} ${vp.name} ${url}:\n${contrastIssues.join('\n')}`).toEqual([]);
                });
            });
        }
    }
