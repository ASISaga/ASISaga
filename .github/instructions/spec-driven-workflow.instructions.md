...
- [ ] Read all provided code, documentation, tests, and logs.
      ...
      - **MCP Tools**: Use `analyze_existing_project` for automated codebase analysis, `parse_existing_documentation` to extract information from README/docs, `extract_requirements_from_code` to mine implicit requirements from implementation
- [ ] Define requirements in **EARS Notation**:
      ...
      - **MCP Tools**: Use `generate_standardized_spec` to create EARS-formatted requirements from analysis results
- [ ] Identify dependencies and constraints.
      - Document a dependency graph with risks and mitigation strategies.
      - **MCP Tools**: Use `analyze_feature_component` to understand feature boundaries and dependencies, `resolve_feature_dependencies` for complex dependency chains
- [ ] Map data flows and interactions.
      - Document system interaction diagrams and data models.
      - **MCP Tools**: Use `extract_feature_boundaries` to identify component interactions in existing codebases
- [ ] Catalog edge cases and failures.
      - Document a comprehensive edge case matrix and potential failure points.
      - **MCP Tools**: Analysis tools automatically identify error handling patterns and edge cases in existing code
- [ ] Assess confidence.
      ...
      - **Decision**: High (>85%) → full implementation; Medium (66-85%) → PoC first; Low (<66%) → research phase
...
- [ ] **Document technical design in `design.md`:**
  ...
  - **MCP Tools**: Use `create_new_feature` for greenfield projects to scaffold design artifacts; use `onboard_project_feature` for brownfield features to generate design from existing code

- [ ] **Document error handling:**
  - Create an error matrix with procedures and expected responses.
  - **MCP Tools**: Analysis tools identify existing error handling patterns

- [ ] **Define unit testing strategy.**
  - **Constitutional Compliance**: Article III (Test-First Imperative) requires TDD
  - **MCP Tools**: Use `setup_plan` to generate implementation and testing plan

- [ ] **Create implementation plan in `tasks.md`:**
  - For each task, include description, expected outcome, and dependencies.
  - **MCP Tools**: Use `create_migration_plan` for brownfield projects, `create_progressive_migration_plan` for large-scale migrations, `setup_plan` for greenfield features
...
- [ ] Code in small, testable increments.
      ...
      - **Constitutional Compliance**: Article I (Library-First) requires each feature as standalone library; Article III (Test-First) requires tests before implementation
- [ ] Implement from dependencies upward.
      - Document resolution order, justification, and verification.
      - **MCP Tools**: Use `resolve_feature_dependencies` to determine correct implementation order
- [ ] Follow conventions.
      - Document adherence and any deviations with a Decision Record.
      - **Constitutional Compliance**: Article VIII (Anti-Abstraction) mandates using frameworks directly without over-engineering
- [ ] Add meaningful comments.
      - Focus on intent ("why"), not mechanics ("what").
- [ ] Create files as planned.
      - Document file creation log.
      - **MCP Tools**: `create_new_feature` scaffolds initial file structure for greenfield features
- [ ] Update task status in real time.
      - **MCP Tools**: Use `track_onboarding_progress` to monitor progressive migration status
...
- [ ] Execute automated tests.
      ...
      - **Constitutional Compliance**: Article IX (Integration-First Testing) requires real databases, not mocks
- [ ] Perform manual verification if necessary.
      - Document procedures, checklists, and results.
      - **MCP Tools**: Use `validate_specification_consistency` to ensure specs align with implementation
- [ ] Test edge cases and errors.
      - Document results and evidence of correct error handling.
      - **MCP Tools**: Validation tools verify EARS requirements are testable and tested
- [ ] Verify performance.
...
- [ ] Refactor for maintainability.
      ...
      - **Constitutional Compliance**: Article VII (Simplicity) limits to ≤3 projects initially
      - **MCP Tools**: Use `detect_specification_conflicts` to identify inconsistencies that need refactoring
- [ ] Update all project documentation.
      - Ensure all READMEs, diagrams, and comments are current.
      - **MCP Tools**: Use `merge_feature_specifications` to consolidate documentation after progressive onboarding
- [ ] Identify potential improvements.
      - Document backlog with prioritization.
      - **MCP Tools**: Use `track_onboarding_progress` to identify areas needing attention
- [ ] Validate success criteria.
      - Document final verification matrix.
      - **MCP Tools**: Use `validate_specification_consistency` for final spec-to-code alignment check
- [ ] Perform meta-analysis.
      - Reflect on efficiency, tool usage, and protocol adherence.
      - **Constitutional Review**: Verify compliance with all 9 Articles (I, III, VII, VIII, IX, etc.)
...
- [ ] Generate executive summary.
      ...
      - **MCP Tools**: Include output from `track_onboarding_progress` for brownfield projects
- [ ] Prepare pull request (if applicable):
    1. Executive summary.
    2. Changelog from **Streamlined Action Log**.
    3. Links to validation artifacts and Decision Records.
    4. Links to final `requirements.md`, `design.md`, and `tasks.md`.
    5. Constitutional compliance report (Articles I, III, VII, VIII, IX)
    6. Specification consistency validation results
- [ ] Finalize workspace.
      - Archive intermediate files, logs, and temporary artifacts to `.agent_work/`.
      - Preserve spec files in `/spec/` or `/specs/` directory
      - **MCP Tools**: Progressive onboarding artifacts stored in `spec/progressive-onboarding/`
- [ ] Continue to next task.
      - Document transition or completion.
      - **MCP Tools**: For progressive migration, use `track_onboarding_progress` to identify next feature
